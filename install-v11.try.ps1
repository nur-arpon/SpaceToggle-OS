...
CreateGuideHUD() {
    ...
    GuideHUD.Add("Text", "Left x" Scale(40) " w" Scale(300) " y" Scale(115), "[Space + Backtick] Multi-Corner PiP Mode")
    GuideHUD.Add("Text", "Left x" Scale(40) " w" Scale(300) " y" Scale(135), "[Space + ,] Input Auto-Focus")
    GuideHUD.Add("Text", "Left x" Scale(40) " w" Scale(300) " y" Scale(155), "[Space + Scroll] Smart Window Opacity")
...
SmartCascade(TargetApp:="", TargetWeb:="", FoundersApp:="", FoundersWeb:="") {
    ...
    if (TargetApp != "") {
        ...
        resolved := ResolvePath(TargetApp)
        if (resolved != "") {
            try { 
...
FocusInputEngine() {
    ...
    try {
        ...
        if InStr(procName, "WhatsApp") {
            Send("^f")
            Sleep(50)
            Send("{Esc}")
        } else if InStr(procName, "chrome") || InStr(procName, "Brave") || InStr(procName, "msedge") || InStr(procName, "firefox") {
            Send("^l")
        ...
        CreateNotificationHUD("🎯 Field Auto-Focused")
    } catch Error {
        return
    }
}
...
*RAlt:: {
    global
    SpaceAborted := true
    ProfileIndex := (ProfileIndex >= ProfilesList.Length) ? 1 : ProfileIndex + 1
    ActiveProfile := ProfilesList[ProfileIndex]
...
Get-Content $PSCommandPath -ErrorAction SilentlyContinue | Set-Content "install.v.11.ps1" -Encoding UTF8 -Force -ErrorAction SilentlyContinue

if (Test-Path ".git") {
    Write-Host "📤 Step 6: Automatically pushing changes back to GitHub cloud..." -ForegroundColor Green
