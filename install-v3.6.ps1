# ==============================================================================
# SpaceToggle OS (Universal Setup Script - V3.6.2 Ultimate Syntax)
# ==============================================================================

# 0. Clean old instances out of active RAM
...
# 3. Create the Patched SpaceToggle Script
$scriptContent = @"
#Requires AutoHotkey v2.0
SetTitleMatchMode 2

; Auto-Elevate to Administrator to conquer Privilege Clashing
if !A_IsAdmin {
    try {
...
; Centralized Global Path Matrix for "Chaos Apps" (V3.6.2 Fixed Matrix)
GetAppPath(exeTarget) {
    switch exeTarget {
        case "brave.exe":
            paths := [A_ProgramFiles "\BraveSoftware\Brave-Browser\Application\brave.exe", A_LocalAppData "\BraveSoftware\Brave-Browser\Application\brave.exe", "C:\Program Files (x86)\BraveSoftware\Brave-Browser\Application\brave.exe"]
        case "chrome.exe":
            paths := [A_ProgramFiles "\Google\Chrome\Application\chrome.exe", "C:\Program Files (x86)\Google\Chrome\Application\chrome.exe", A_LocalAppData "\Google\Chrome\Application\chrome.exe"]
        case "Discord.exe":
            paths := [A_LocalAppData "\Discord\Update.exe", A_ProgramFiles "\Discord\Discord.exe", A_AppData "\Discord\Update.exe"]
        case "Spotify.exe":
            paths := [A_AppData "\Spotify\Spotify.exe", A_LocalAppData "\Microsoft\WindowsApps\Spotify.exe", A_ProgramFiles "\Spotify\Spotify.exe"]
        case "WhatsApp.exe":
            paths := [A_LocalAppData "\WhatsApp\WhatsApp.exe", A_LocalAppData "\Microsoft\WindowsApps\WhatsApp.exe", A_ProgramFiles "\WhatsApp\WhatsApp.exe"]
        case "Zoom.exe":
            paths := [A_AppData "\Zoom\bin\Zoom.exe", A_LocalAppData "\Zoom\bin\Zoom.exe", A_ProgramFiles "\Zoom\bin\Zoom.exe"]
        default:
            return ""
    }
...
SmartLaunch(exeTarget, runCommand, webFallback := "") {
    ...
    resolvedPath := GetAppPath(exeTarget)
    if (resolvedPath != "") {
        try {
            if (exeTarget = "Discord.exe" && InStr(resolvedPath, "Update.exe")) {
                Run('"' resolvedPath '" --processStart Discord.exe')
            } else {
                Run('"' resolvedPath '"')
            }
            return
    ...
    if InStr(runCommand, ":") {
        proto := StrSplit(runCommand, ":")[1]
        try {
            RegRead("HKEY_CLASSES_ROOT\" proto)
        } catch {
            if (webFallback != "") {
...
OpenInBrowser(url) {
    bravePath := GetAppPath("brave.exe")
...
Write-Host "SUCCESS! SpaceToggle OS V3.6 Matrix is installed and running." -ForegroundColor Green
